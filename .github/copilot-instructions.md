...
Purpose: Enable AI agents to quickly extend and maintain the Digital Cookbook (Bun + React + Vite + Express + SQLite) without re-discovering core patterns.
...
- Backend: Lightweight Express server (`server/index.cjs`) exposing JSON REST under `/api/*`, using Bun's native SQLite driver (`bun:sqlite`) with the file DB in `data/cookbook.db`. WAL + FK pragmas enabled.
...
## 3. Key Backend Endpoints (see exact SQL in `server/index.cjs`)
GET /api/health
...
- New API route: Implement in `server/index.cjs` (CommonJS). Keep small, synchronous DB ops; wrap multi-step mutations in a `bun:sqlite` transaction (`db.transaction(() => {...})`). Return minimal JSON.
...
## 9. Future Tasks (see `todo.md`)
When implementing an item, add a checkmark to `todo.md` and include concise rationale in PR/commit.

## 10. Quick Decision Rules
